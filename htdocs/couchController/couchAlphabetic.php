...
foreach ($title->{rows} as $value){
  $doc = "";  
  $doc['title'] =      $value->{doc}->{v100}[0]->{_};
  $doc['issn'] =       $value->{doc}->{v400}[0]->{_};
  $doc['issn2'] =      $value->{doc}->{v935}[0]->{_};
  $doc['issn2_type'] = $value->{doc}->{v35}[0]->{_};

  if (array_key_exists($doc['issn'],$count_dict)){
      $doc['issues_total'] = $count_dict[$doc['issn']];
  }else{
      $doc['issues_total'] = 0;
  }
  ...
  if ($value->{doc}->{v50}[0]->{_} == "C") {
    if (array_key_exists("v51",$value->{doc})){
      foreach ($value->{doc}->{v51} as $valueb)
        ...
        array_push($doc['history'],$history);
    }
  }
  array_push($docs,$doc);
} //foreach end
